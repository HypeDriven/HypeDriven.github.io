  ...
  float starLayer(vec2 uv, float scale, float drift, float t) {
    ...
    for (int y = -1; y <= 1; y++) {
      for (int x = -1; x <= 1; x++) {
        ...
        float visible = step(0.7, brightness);
        brightness = brightness * brightness * visible;
        float twinkle = 0.85 + 0.15 * sin(t * 0.5 + brightness * 40.0);
        float star = brightness * twinkle * smoothstep(0.05, 0.0, dist);
        star += brightness * twinkle * 0.15 * smoothstep(0.15, 0.0, dist);
        stars += star;
      }
    }
    return stars;
